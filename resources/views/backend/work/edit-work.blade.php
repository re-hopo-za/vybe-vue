    ...
    <div class="container-fluid">
        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <!-- Content Row -->
                <form action="{{ route('work.workUpdate', ['work' => $work]) }}" method="POST" class="row" enctype="multipart/form-data">
                    @method('PATCH')
                    @csrf
                        ...
                        <div class="row">
                            <div class="col-12 col-lg-6">
                                <div class="form-group">
                                    <label for="work_cover">تصویر</label>
                                    <input type="file" class="dropify" name="work_cover" id="work_cover" data-height="280"
                                           data-max-file-size="3M" data-allowed-file-extensions="jpeg jpg png"
                                           data-default-file="{{ isset($work) ? $work->getFirstMediaUrl('work_cover') : null }}"/>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-sm-6 col-lg-3">
                        <div class="form-label-group">
                            <select class="form-control custom-select-lg b-select @error('category_id') is-invalid @enderror" id="category_id"
                                ...
                                <optgroup label="Work's Categories">
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ $work->category_id == $cat->id ? 'selected' : '' }}>{{ $cat->title }}</option>
                                @endforeach
                                </optgroup>
                            </select>
                    ...
                    <div class="col-12 col-sm-6 col-lg-3">
                        <div class="form-label-group">
                            <select class="form-control custom-select-lg b-select @error('tag') is-invalid @enderror" id="tags"
                                ...
                                @foreach($tags as $tag)
                                    <option value="{{ $tag->id }}"
                                            @foreach($work->tags as $work_tag)
                                            {{ $tag->id == $work_tag->id ? 'selected' : '' }}
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            <label for="text">متن کامل نمونه کار</label>
                            <textarea name="text" id="text" class="text" rows="5" tabindex="5">{!! $work->text !!}</textarea>
                        </div>
                    </div>

                    <div class="col-12">
                        <a class="btn btn-outline-secondary mb-1 mb-sm-0" role="button" href="{{ route('work.workList') }}" tabindex="6">بازگشت</a>
...
@section('page-script')
    ...
    <script src="{{ asset('js/backend/work/work-scripts.min.js') }}"></script>
@endsection
